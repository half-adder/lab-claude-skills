<overview>
Cluster-specific SLURM configuration. All values below use placeholders from `cluster-config.toml`. Read that file first and substitute the actual values.
</overview>
...
- CPUs: match GPU node topology (typically 8-12)
- Memory: 20-32GB (more if loading large datasets)
- Time: estimate based on epochs/data, add 50% buffer
- GPU: 1 (multi-GPU rarely needed for microscopy)

**CPU jobs (preprocessing, analysis):**
